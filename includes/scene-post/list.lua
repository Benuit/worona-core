...
local createButton = require( "extensions.scene-post.button" )
...
local function createList( field )
	...
	local list_sheet_options = {
     ...
     local list_sheet = graphics.newImageSheet( images_folder .. "listSheet.png", list_sheet_options[theme] )
	...
	for i = 1, #field.list do
		...
		row.text                  = "   " .. row.text
		row.text_color            = { default = { 0.2, 0.2, 0.2, 1 }, over = { 0, 0, 0, 1 } }
		row.reset_font            = true
		row.sheet                 = list_sheet

		createButton( row )

		local arrow = display.newImageRect( images_folder .. "listArrow.png", 14, 22 )
